...
- (void)viewDidLoad {
    ...
    self.tableView.tableHeaderView = ({
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 320, _webView.height+ 5)];
...
-(void)shareVideoInScene:(enum WXScene)scene{
    ...
    NSString *url = [@"http://xianng.com/videoplay/video.html?videoid=" stringByAppendingString:_videoObject.ellenId];
    [[AFImageDownloader defaultInstance]downloadImageForURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_videoObject.thumbnail]] success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull responseObject) {
        [WXApiRequestHandler sendLinkURL:url TagName:@"EllenGif" Title:content Description:content ThumbImage:responseObject InScene:(enum WXScene)scene];
    } failure:nil];
